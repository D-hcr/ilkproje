<template>
    <v-container id="simge">
        <a id="simgein" href="https://aquadipolo.com/pages/ucretsiz-kargo" title="">
            
            <v-icon class="policy-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-truck" viewBox="0 0 16 16">
                   <path d="M0 3.5A1.5 1.5 0 0 1 1.5 2h9A1.5 1.5 0 0 1 12 3.5V5h1.02a1.5 1.5 0 0 1 1.17.563l1.481 1.85a1.5 1.5 0 0 1 .329.938V10.5a1.5 1.5 0 0 1-1.5 1.5H14a2 2 0 1 1-4 0H5a2 2 0 1 1-3.998-.085A1.5 1.5 0 0 1 0 10.5v-7zm1.294 7.456A1.999 1.999 0 0 1 4.732 11h5.536a2.01 2.01 0 0 1 .732-.732V3.5a.5.5 0 0 0-.5-.5h-9a.5.5 0 0 0-.5.5v7a.5.5 0 0 0 .294.456zM12 10a2 2 0 0 1 1.732 1h.768a.5.5 0 0 0 .5-.5V8.35a.5.5 0 0 0-.11-.312l-1.48-1.85A.5.5 0 0 0 13.02 6H12v4zm-9 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm9 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
                </svg>
            </v-icon>
            

            <v-text-field class="policy-text">
              
            <a id="itemx" class="item-title">
            ÜCRETSİZ Kargo*
            </a>

            </v-text-field>   
        </a>
        <a id="simgein" href="https://aquadipolo.com/pages/ucretsiz-kargo" title="">
            
            <v-icon class="policy-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-box-seam" viewBox="0 0 16 16">
                    <path d="M8.186 1.113a.5.5 0 0 0-.372 0L1.846 3.5l2.404.961L10.404 2l-2.218-.887zm3.564 1.426L5.596 5 8 5.961 14.154 3.5l-2.404-.961zm3.25 1.7-6.5 2.6v7.922l6.5-2.6V4.24zM7.5 14.762V6.838L1 4.239v7.923l6.5 2.6zM7.443.184a1.5 1.5 0 0 1 1.114 0l7.129 2.852A.5.5 0 0 1 16 3.5v8.662a1 1 0 0 1-.629.928l-7.185 2.874a.5.5 0 0 1-.372 0L.63 13.09a1 1 0 0 1-.63-.928V3.5a.5.5 0 0 1 .314-.464L7.443.184z"/>
                </svg>
            </v-icon>
            

            <v-text-field class="policy-text">
              
            <a id="itemx" class="item-title">
            
                Aynı Gün Kargo*
            </a>

            </v-text-field>   
        </a>
        <a id="simgein" href="https://aquadipolo.com/pages/ucretsiz-kargo" title="">
            
            <v-icon class="policy-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">
            <path d="M11.534 7h3.932a.25.25 0 0 1 .192.41l-1.966 2.36a.25.25 0 0 1-.384 0l-1.966-2.36a.25.25 0 0 1 .192-.41zm-11 2h3.932a.25.25 0 0 0 .192-.41L2.692 6.23a.25.25 0 0 0-.384 0L.342 8.59A.25.25 0 0 0 .534 9z"/>
            <path fill-rule="evenodd" d="M8 3c-1.552 0-2.94.707-3.857 1.818a.5.5 0 1 1-.771-.636A6.002 6.002 0 0 1 13.917 7H12.9A5.002 5.002 0 0 0 8 3zM3.1 9a5.002 5.002 0 0 0 8.757 2.182.5.5 0 1 1 .771.636A6.002 6.002 0 0 1 2.083 9H3.1z"/>
            </svg>
            </v-icon>
            

            <v-text-field class="policy-text">
              
            <a id="itemx" class="item-title">
            
                Ücretsiz ve Kolay İADE
            </a>

            </v-text-field>   
        </a>
    </v-container>        
                    
      
</template>

<style>
#simge {
    position: relative;
    top: 180px;
    left: 300px;
    
}
.policy-icon {
    margin-right: 10px;
    color: black;
    position: relative;
    top: -3px;
    
}
#itemx {
    color: black;
    text-decoration-line: none;
}
#simgein {
    text-decoration-line: none;
    margin-right: 120px;
    
}
#simgein:hover {
  text-decoration-line: underline;
}

</style>