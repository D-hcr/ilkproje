<template>
     <v-container id="ct" class="container">
    <v-text-field class="title" > 
                
                <span >
                   Dikkat Çeken Ürünler
                </span>
  
    </v-text-field>
    <span id="tümg">
        <a id="tümg2" href="#">Tümünü Gör</a>
    </span>
    <v-carousel  id="carouselExampleControlsiki"  class="carousel slide"  data-bs-ride="carousel" >
  <section class="carousel-inner" >
    <v-carousel-item class="carousel-item active">
        <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem; ">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Kadin-Gumus-Metal-Kordonlu-Yildiz-Desenli-Dijital-Retro-Saat-APWA063800.jpg?v=1653899466" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo  Rose Gold ve Gümüş Metal Kordonlu Dijital Retro KolSaati 2'li Hediye Seti</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Unisex-Siyah-Metal-Kordonlu-Mermer-Desenli-Dijital-Retro-Saat-APWA064300.jpg?v=1653899094" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Unisex Siyah Metal Kordonlu Mermer Desenli Dijital Retro Saat </p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Kadin-Rose-Gold-Metal-Kordonlu-Cicek-Desenli-Dijital-Retro-Saat-APWA063801.jpg?v=1653899814" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Kadın Rose Gold Metal Kordonlu Çiçek Desenli Dijital Retro Saat</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Kadin-Altin-Metal-Kordonlu-Kurukafa-Desenli-Dijital-Retro-Saat-APWA063400.jpg?v=1653899219" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Kadın Altın Metal Kordonlu Kurukafa Desenli Dijital Retro Saat</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
    </v-carousel-item>
    <v-carousel-item class="carousel-item">
        <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Gran-Paradiso-Sport-50-Ml-EDP-Erkek-Parfum-APCN0005.jpg?v=1649181678" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Gran Paradiso Sport 50 Ml EDP Erkek Parfüm </p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Gran-Paradiso-Red-50-Ml-EDP-Erkek-Parfum-APPPGR03EP.jpg?v=1649181282" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Gran Paradiso Red 50 Ml EDP Erkek Parfüm</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Gran-Paradiso-Oasis-50-Ml-EDP-Erkek-Parfum-APCN000504.jpg?v=1649187210" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Gran Paradiso Oasis 50 Ml EDP Erkek Parfüm </p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Gran-Paradiso-Volcano-Sport-50-Ml-EDP-Erkek-Parfum-APCN000512.jpg?v=1649181488" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Gran Paradiso Volcano Sport 50 Ml EDP Erkek Parfüm</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
    </v-carousel-item>
    <v-carousel-item class="carousel-item">
        <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Unisex-Mor-Premium-Sirt-Cantasi-APBA012805_360x.png?v=1649220798" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Unisex Mor Premium Sırt Çantası</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/71965_1_360x.jpg?v=1667403707" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo 1987 APBA044001 Kadın Bej Omuz Çantası</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Kadin-Siyah-Omuz-Askili-Telefon-Cantasi-APBA041001_360x.jpg?v=1652862742" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Kadın Siyah Omuz Askılı Telefon Çantası</p>
                <p id="price">499.00 TL</p>
            </v-textarea>
       </div>
       <div  id="pcorusel" class="card" style="width: 15rem; height: 23rem;">
            <v-img id="rid">
                <img id="resim" src="https://cdn.shopify.com/s/files/1/0485/5264/0672/products/Aqua-di-Polo-Kadin-Krem-Omuz-Cantasi-APBA014004_360x.jpg?v=1650373610" class="card-img-top" alt="...">
            </v-img>
            <v-textarea class="card-body">
                <p class="card-text">Aqua di Polo Kadın Krem Omuz Çantası</p>
                <p id="price" >499.00 TL</p>
            </v-textarea>
       </div>
    </v-carousel-item>
  </section>
  <button id="button1" class="carousel-control-prev" type="button" style="color:black" data-bs-target="#carouselExampleControlsiki" data-bs-slide="prev">
    <span  class="carousel-control-prev-icon" style="color:black" aria-hidden="true"></span>
    <span id="iconx" class="visually-hidden">Previous</span>
  </button>
  <button id="button2" class="carousel-control-next" type="button" data-bs-target="#carouselExampleControlsiki" data-bs-slide="next">
    <span  class="carousel-control-next-icon" aria-hidden="true"></span>
    <span id="iconx" class="visually-hidden">Next</span>
  </button>
    </v-carousel>

     </v-container>
</template>

<style>
.container {
    width: 50%;
    position: relative;
    left: -450px;
    display: flex;
    flex-direction: column;
}
.title {
    margin-top: 47px;
}
#carouselExampleControlsiki{
    position: relative;
    left: 250px;
    top: 50px;
    height: 400px;
    width: 1100px;
    margin-top: 30px;
}
#resim {
    height: 200px;
    width: 150px;
    position: relative;
    left: 40px;
   
}
#button1 {
    background-color: rgb(204, 203, 203);
    height: 50px;
    width: 50px;
    position: absolute;
    left: -60px;
    top: 130px;
    
}
#button2 {
    background-color: rgb(204, 203, 203);
    height: 50px;
    width: 50px;
    position: absolute;
    right: -40px;
    top: 110px;
    
}
#rid {
    border: solid rgb(200, 198, 198);
    position: relative;
    left: -1px;

}
#pcorusel {
    border: none;
    float: left;
    margin-left: 25px;
    height: 300px;
    
}
#price {
    font-weight:bold;
}
#ct {
    margin-top: 30px;
    position: relative;
    top: 100px;
}
#tümg {
    position: relative;
    left: 740px;
    top: 15px;
   
}
#tümg2 {
    text-decoration-line: none;
    font-weight: bold;
    color: #ba7a2d;
}
</style>